...
#### レーダーチャート（政党別の大項目サマリ）
表示の「レーダーチャート」を選択すると、各政党の「大項目（5〜6軸）」のスタンスを多角形で一覧できます。

- **中心:-100 / 外側:+100**（0点は太いリングで表示）
- 多角形（塗り）: **中央値（median）**
- 緑の線: **最大値（max）**
- 赤の線: **最小値（min）**
- 薄い塗り: **最小〜最大の範囲**
- 「レーダーチャートの見方」カードの大項目チップにホバーすると、その大項目に含まれるトピック（プレビュー）を確認できます
