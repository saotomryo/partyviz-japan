...
#### 3.1.2 政党要旨（相対評価）
各政党のトピック別スコアを、他政党との相対関係を含めて100〜150字程度で要約表示する。

- 相対特徴: 各トピックの平均との差（z-score）から **上位/下位の特徴トピック** を抽出
- 近傍/遠方: スコアベクトル距離で **近い政党/遠い政党** を併記
- 根拠: 代表的な引用（evidence quote）を短く添える
- 財政など方向性が誤解されやすいトピックは注釈を付与（例: 「財政はマイナス側=積極財政寄り」）

#### 3.1.1 政党別の大項目サマリ（レーダー用）
多数トピックをそのままレーダー化すると読みにくくなるため、トピックを5〜6程度の大項目に分類して集約表示する。

- 大項目例: 経済・財政 / 社会保障・子育て / 外交・安全保障 / 人権・多様性 / デジタル・行政改革 / その他
- 大項目スコア:
  - 代表値: **中央値（median）**
  - ブレ: **min〜max（ブレ幅）** を併記
  - **スコアなし（未評価/根拠不足）** は集約から除外
  - ブレが大きい場合は「大項目内で立場が一貫していない可能性」を示す指標として扱う
 - 表示（レーダーチャート）:
   - 中心:-100 / 外側:+100（0点は強調リング）
   - 多角形（塗り）: 中央値（median）
   - 最大（線）/ 最小（線）: max / min を重ねて表示
   - 範囲（塗り）: min〜max の帯を薄く塗りつぶす
※ 大項目の分類（どのトピックがどの大項目に入るか）はUI上で参照できる（ホバー表示）。
...
- `GET /summaries/parties?scope=official|mixed`（政党要旨の相対評価）
