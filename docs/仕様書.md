...
- `POST /admin/resolve/run`（通常はキューで自動）  
- `POST /admin/crawl/run`  
- `POST /admin/score/run`  
- `POST /admin/sources`（手動レビュー登録・修正用）
...
## 13. フロントエンド技術選定（MVP → リッチUI）

- 採用方針: **Vite + React + TypeScript** をフレームとし、グラフ描画は **Apache ECharts** を採用。理由は以下。  
  - 動的な1軸/2軸切替・レンジ表示・ツールチップ等のリッチ表現を少ないコードで実装しやすい。  
  - EChartsは散布図・レンジ帯（custom series）・ライン/ヒートマップ等が充実しており、日本語フォント/RTLも扱いやすい。  
  - Vite + TS でホットリロードと型安全性を確保しつつ、構築コストが低い。  
- 状態管理: **Zustand**（軽量で学習コストが低い）。将来グローバル状態が増えた場合も拡張容易。  
- スタイル: **CSS Modules or Tailwind CSS** のいずれかを選択（初期はCSS Modulesで開始し、コンポーネント増加時にTailwindへ移行を検討）。  
- API連携: `.env` に `VITE_API_BASE` を設定し、バックエンドFastAPIと疎通。  
- ビルド/配備: 静的ビルドを行い、CDNもしくは静的ホスティングに配置。バックエンドとはCORSまたはリバースプロキシで連携。
