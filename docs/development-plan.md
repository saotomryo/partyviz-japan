...
## 5.1 レーダーチャート方針（案）
政党ごとのスコアをそのままレーダーにすると「軸が多すぎて読めない」ため、まずは以下の方針を検討する。

1) 大項目（5〜6カテゴリ）に集約（実装済み/採用）
   - 各トピックを「経済・財政 / 社会保障・子育て / 外交・安全保障 / 人権・多様性 / デジタル・行政改革 / その他」に分類
   - 大項目ごとに **中央値** を代表値、**ブレ幅（min〜max）** を併記（スコアなしは除外）
   - 目的: 党内/トピック間で立場が一貫しているか（ブレの有無）も同時に把握

2) 2枚レーダー（立場×情報量）に分ける（将来）
   - 立場（中央値）とは別に、情報量/確度を「対象トピック数」「平均confidence」「根拠件数」などで可視化
   - 目的: 立場の強さだけでなく「どれだけ一次情報があるか」を区別して誤読を減らす

3) クラスタ/プロファイルへの射影（将来）
   - トピック群のベクトルを用いて「典型プロファイル（例: 市場重視/再分配重視/安全保障重視 等）」への近さをスコア化
   - 目的: 多数トピックを「政策スタイル」として俯瞰しやすくする（レーダーの軸を固定化できる）
