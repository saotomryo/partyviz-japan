...
- 設計
  - [ ] `docs/policy-indexing-spec.md` の確定
  - [ ] 公式/混合のソース優先順位（インデックス vs 検索）の整理
  - [ ] index_only（インデックスのみ評価）の導入
- 管理UI
  - [ ] 政党編集に `policy_base_urls` を追加（複数URL）
  - [ ] クロール実行/ステータス表示（HTML/PDF件数・エラー件数）
- 公開UI
...
- インデックス
  - [ ] HTML/PDF本文抽出
  - [ ] チャンク化と埋め込み（pgvector or ローカル）
  - [ ] 文書/チャンクのメタ保存（URL/タイトル/取得日時）
- スコアリング統合
  - [ ] official: インデックス優先、検索は補助
  - [ ] mixed: インデックス + 検索の併用
  - [ ] 根拠URL実在チェックの維持
